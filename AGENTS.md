...
- **macOS, Windows, and Ubuntu (Linux) are the officially supported MVP platforms.** Linux/Wayland has known tray and popover-positioning quirks, confirmed fixes, and dead ends already tried — see [docs/linux-wayland-notes.md](docs/linux-wayland-notes.md) before touching `src/tray-platform.ts` or popover positioning.
